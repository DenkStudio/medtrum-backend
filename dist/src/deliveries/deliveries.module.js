"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.DeliveriesModule = void 0;
const common_1 = require("@nestjs/common");
const deliveries_admin_service_1 = require("./deliveries.admin.service");
const deliveries_admin_controller_1 = require("./deliveries.admin.controller");
const users_module_1 = require("../users/users.module");
let DeliveriesModule = class DeliveriesModule {
};
exports.DeliveriesModule = DeliveriesModule;
exports.DeliveriesModule = DeliveriesModule = __decorate([
    (0, common_1.Module)({
        imports: [users_module_1.UsersModule],
        providers: [deliveries_admin_service_1.DeliveriesAdminService],
        controllers: [deliveries_admin_controller_1.DeliveriesAdminController],
        exports: [deliveries_admin_service_1.DeliveriesAdminService],
    })
], DeliveriesModule);
//# sourceMappingURL=deliveries.module.js.map